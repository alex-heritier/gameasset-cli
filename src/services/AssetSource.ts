...
export interface AssetSource {
  ...
  getAssetFileInfo(assetUrl: string): Promise<{
    fileType?: string;
...
export abstract class BaseAssetSource implements AssetSource {
  ...
  async getAssetFileInfo(assetUrl: string): Promise<{
    fileType?: string;
    download?: { url: string; filename: string };
  }> {
    ...
    const fileType = download.filename.split(".").pop()?.toUpperCase();
