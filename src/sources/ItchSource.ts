...
export class ItchSource extends BaseAssetSource {
  ...
  parseSearchResults(html: string, limit: number): Asset[] {
    ...
    $(".game_cell").each((i, elem) => {
      ...
      const cover =
        $elem.find(".game_thumb img").attr("data-lazy_src") ||
        $elem.find(".game_thumb img").attr("src");
  ...
  async extractDownloadUrl(
    ...
    if (downloadBtn.length > 0) {
      ...
      if (href)
        return { url: "https://itch.io" + href, filename: "download.zip" };
    }
    ...
    if (uploadNames.length > 0) {
      const filename =
        uploadNames.first().attr("title") || uploadNames.first().text().trim();
      if (filename) {
