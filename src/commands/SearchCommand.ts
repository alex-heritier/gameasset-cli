...
export class SearchCommand {
  ...
  register(): void {
    this.program
      ...
      .option(
        "-s, --source <source>",
        `Source to search (all sources if not specified)`,
      )
  ...
  private async execute(options: SearchCommandOptions): Promise<void> {
    const limit = Math.min(
      parseInt(options.limit, 10) || DEFAULT_SEARCH_LIMIT,
      MAX_SEARCH_LIMIT,
    );
    ...
    try {
      const sources = options.source
        ? [options.source]
        : this.repository.getAvailableSources();
      ...
      const allResults: Asset[] = searchResults.flatMap(
      ...
      if (options.json) {
        console.log(
          JSON.stringify(
            { assets: allResults, totalFound: allResults.length },
            null,
            2,
          ),
        );
