...
describe('ItchSource E2E Tests', () => {
  ...
  it('should build search URL correctly', () => {
    ...
    expect(url).toBe('https://itch.io/game-assets/free?q=test%20assets&tag=2d&tag=sprites');
  });
  ...
  it('should extract download URL from asset page', async () => {
    ...
    const result = await source.extractDownloadUrl(html, 'https://itch.io/game/123');
    expect(result).toEqual({ url: 'https://itch.io/download/123', filename: 'download.zip' });
  });
  ...
  it('should handle full search and download flow', async () => {
    ...
    const downloadResult = await source.extractDownloadUrl(assetHtml, asset.link);
    expect(downloadResult).toEqual({ url: 'https://itch.io/download/456', filename: 'download.zip' });

    // Mock download
    const download = await source.downloadAsset(downloadResult!.url, '/tmp/test.zip');
